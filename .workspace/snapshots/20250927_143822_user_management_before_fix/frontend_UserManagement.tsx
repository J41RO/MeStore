...
const UserManagement: React.FC = () => {
  ...
  const loadUserData = async () => {
    try {
      ...
      // Cargar estadísticas
      try {
        const statsResponse = await fetch('http://192.168.1.137:8000/api/v1/user-management/stats', {
          headers: {
      ...
      const usersResponse = await fetch('http://192.168.1.137:8000/api/v1/user-management/users?limit=50', {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
  ...
  const performUserAction = async (userId: string, action: string, reason?: string) => {
    try {
      ...
      const response = await fetch(`http://192.168.1.137:8000/api/v1/user-management/users/${userId}/action`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
