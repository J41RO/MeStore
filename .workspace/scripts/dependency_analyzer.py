...
class DependencyAnalyzer:
    def __init__(self, project_root: str = "."):
        ...
        self.framework_patterns = {
            ...
            'database': ['sqlalchemy', 'psycopg2', 'mysql', 'sqlite', 'redis', 'chromadb'],
            'api_client': ['requests', 'httpx', 'aiohttp'],
            'data_processing': ['pandas', 'numpy', 'matplotlib', 'seaborn'],
            'ml_ai': ['sklearn', 'tensorflow', 'torch', 'transformers', 'sentence_transformers'],
    ...
    def _analyze_code_patterns(self, content: str, tree: ast.AST, file_path: Path) -> Dict:
        ...
        analysis["domain_type"] = self._determine_domain_type(detected_frameworks, content)
        
        # Análisis de casos edge
        analysis["edge_cases"] = self._analyze_edge_cases(tree, content)
        
        # Consideraciones de performance
        analysis["performance_considerations"] = self._analyze_performance_patterns(tree, content)
        
        # Patrones de error
        analysis["error_patterns"] = self._analyze_error_patterns(tree, content)
        
        # Puntos de integración
        analysis["integration_points"] = self._analyze_integration_points(tree, content, all_imports)
    ...
    def _determine_domain_type(self, frameworks: List[str], content: str) -> str:
        ...
        if any(fw in frameworks for fw in ['fastapi', 'django', 'flask']):
            ...
            elif any(word in content.lower() for word in ['router', 'endpoint', 'route']):
                return 'web_api'
    ...
    def _analyze_performance_patterns(self, tree: ast.AST, content: str) -> List[str]:
        ...
        if loop_depth > 1:
            performance.append(f"Loops anidados (profundidad: {loop_depth}) - testear con datasets grandes")
        
        # Operaciones async
        if 'async' in content or 'await' in content:
            performance.append("Operaciones asíncronas - testear timeouts y concurrencia")
        ...
        if any(word in content for word in ['requests.', 'httpx.', 'urllib']):
            performance.append("Llamadas de red - testear latencia y fallos de conexión")
    ...
    def _analyze_error_patterns(self, tree: ast.AST, content: str) -> List[str]:
        ...
        exceptions = []
        for node in ast.walk(tree):
            if isinstance(node, ast.Raise) and hasattr(node, 'exc') and node.exc is not None:
                exc = node.exc
        ...
        if exceptions:
            error_patterns.append(f"Excepciones específicas: {', '.join(set(exceptions))}")
    ...
    def _analyze_integration_points(self, tree: ast.AST, content: str, imports: List[str]) -> List[str]:
        ...
        return {
            ...
            "summary": {
                ...
                "external_dependencies": sorted(list(set(
                    dep for analysis in files_analysis.values() 
                    for dep in analysis.get("external_imports", [])
                ))),
                "files_to_provide_to_ai": [
                    rel_file for rel_file in related_files 
...
def main():
    if len(sys.argv) < 2:
        print("Uso: python dependency_analyzer.py <archivo_python> [directorio_proyecto]")
    ...
    print(f"📍 Ruta sugerida para test: {result['main_file']['suggested_test_path']['recommended']}")
    ...
    if 'detected_frameworks' in result['main_file'] and result['main_file']['detected_frameworks']:
...
{chr(10).join(f"   - {dep}" for dep in result['summary']['external_dependencies'])}"""
    ...
    if all_strategies_for_prompt:
        ...
        instructions += chr(10).join(f"   - {strategy}" for strategy in sorted(all_strategies_for_prompt))
    ...
    if advanced.get('performance_considerations'):
        ...
        instructions += chr(10).join(f"   - {perf}" for perf in advanced['performance_considerations'])
    
    if advanced.get('error_patterns'):
        instructions += f"\n\n🚨 PATRONES DE ERROR A VALIDAR:\n"
        instructions += chr(10).join(f"   - {error}" for error in advanced['error_patterns'])
    
    if advanced.get('integration_points'):
        instructions += f"\n\n🔄 PUNTOS DE INTEGRACIÓN A MOCKEAR:\n"
        instructions += chr(10).join(f"   - {integration}" for integration in advanced['integration_points'])
