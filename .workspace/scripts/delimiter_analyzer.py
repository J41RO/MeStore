...
class DelimiterAnalyzer:
    ...
    def analyze_delimiters(self, enabled_delimiters: Dict[str, bool]) -> Dict[str, Any]:
        ...
        analysis_result = {
            "file_info": {
                ...
                "analyzed_delimiters": [
                    name for name, enabled in enabled_delimiters.items() if enabled
                ],
            },
        ...
        if analysis_result["summary"]["errors"] == 0:
            ...
            analysis_result["recommendations"].append(
                "✅ Todos los delimitadores están correctamente balanceados"
            )
        else:
            analysis_result["balance_status"] = "ERROR"
            analysis_result["recommendations"].append(
                f'❌ Se encontraron {analysis_result["summary"]["errors"]} errores críticos'
            )
    ...
    def _find_matching_html_opener(
        self, tag_stack: List[Dict], tag_name: str
    ) -> Optional[Dict]:
    ...
    def _analyze_traditional_delimiters(
        self, analysis_result: Dict[str, Any], enabled_delimiters: Dict[str, bool]
    ):
        ...
        for i, char in enumerate(chars):
            ...
            if self._is_quote_delimiter(char, enabled_delimiters):
                if not in_string:
                    ...
                    opener = self._find_matching_opener(
                        stack, self._get_quote_type(char)
                    )
                    if opener:
                        stack.remove(opener)
                        analysis_result["valid_matches"].append(
    ...
    def _is_quote_delimiter(
        self, char: str, enabled_delimiters: Dict[str, bool]
    ) -> bool:
    ...
    def _find_matching_opener(
        self, stack: List[Dict], delimiter_type: str
    ) -> Optional[Dict]:
    ...
    def display_analysis_report(self, analysis: Dict[str, Any]):
        ...
        print(
            f"🎯 DELIMITADORES ANALIZADOS: {', '.join(analysis['file_info']['analyzed_delimiters'])}"
        )
        ...
        if analysis["errors"]:
            ...
            for i, error in enumerate(analysis["errors"], 1):
                if error["delimiter_type"] == "html_tags":
                    ...
                    print(
                        f"  📍 Ubicación: Línea {error['line']}, Columna {error['column']}"
                    )
        ...
        if analysis["errors"]:
            ...
            for error in analysis["errors"][:3]:  # Mostrar solo los primeros 3
                ...
                print(
                    f"    python3 pattern_extractor.py {self.file_path} \"{error['context']['line_content'].strip()}\" --context=5"
                )
...
def main():
    ...
    parser.add_argument(
        "--parentheses", action="store_true", help="Analizar paréntesis ( )"
    )
    parser.add_argument(
        "--brackets", action="store_true", help="Analizar corchetes [ ]"
    )
    parser.add_argument(
        "--quotes", action="store_true", help='Analizar comillas dobles " "'
    )
    parser.add_argument(
        "--single-quotes", action="store_true", help="Analizar comillas simples ' '"
    )
    parser.add_argument(
        "--html-tags", action="store_true", help="Analizar tags HTML/JSX <div></div>"
    )
    parser.add_argument(
        "--all", action="store_true", help="Analizar todos los delimitadores"
    )
    parser.add_argument(
        "--context", type=int, default=3, help="Líneas de contexto (default: 3)"
    )
    parser.add_argument(
        "--summary-only", action="store_true", help="Mostrar solo resumen"
    )
