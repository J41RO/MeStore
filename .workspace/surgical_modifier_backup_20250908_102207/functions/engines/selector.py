...
class EngineSelector:
    ...
    def get_performance_dashboard(self) -> Dict[str, Any]:
        """
        Genera dashboard comprensivo de métricas de performance.
        
        Returns:
            Dashboard con secciones organizadas de métricas
        """
        dashboard = {
            'overview': self._get_dashboard_overview(),
            'engine_comparison': self._get_engine_comparison_data(),
    ...
    def _get_dashboard_overview(self) -> Dict[str, Any]:
        """Sección overview del dashboard"""
        total_operations = sum(self._performance_metrics.get('operation_counts', {}).values())
        
        return {
            'total_operations': total_operations,
            'cache_hit_rate': self._calculate_cache_hit_rate(),
            'average_selection_time': self._calculate_avg_selection_time(),
            'active_engines': len(EngineRegistry.list_engines()),
            'health_status': self._get_overall_health_status()
        }
    
    def _get_engine_comparison_data(self) -> Dict[str, Any]:
        """Datos comparativos entre engines"""
        comparison_data = {}
        ...
        for engine_name in EngineRegistry.list_engines():
            try:
                engine = EngineRegistry.get_engine(engine_name)
                if hasattr(engine, '_metrics'):
                    metrics = engine._metrics.get_performance_summary()
                    comparison_data[engine_name] = {
                        'success_rate': metrics.get('success_rate', 0.0),
                        'avg_response_time': metrics.get('avg_response_time', 0.0),
                        'total_operations': metrics.get('total_operations', 0),
                        'reliability_score': self.failure_registry.get_reliability_score(engine_name)
                    }
            except Exception:
                comparison_data[engine_name] = {
                    'status': 'unavailable',
    ...
    def _get_performance_trends(self) -> Dict[str, Any]:
        ...
        trends = {
            'selection_times': {
                'recent_avg': self._calculate_recent_avg_selection_time(),
                'trend': self._calculate_selection_time_trend(),
                'peak_times': self._identify_peak_performance_times()
            },
            'engine_usage': self._calculate_engine_usage_trends(),
            'error_rates': self._calculate_error_rate_trends()
        }
        ...
        for engine_name in EngineRegistry.list_engines():
            try:
                ...
                if hasattr(engine, '_metrics'):
                    ...
                    if success_rate < 0.8:
                        alerts.append({
                            ...
                            'message': f'Engine {engine_name} success rate: {success_rate:.1%}',
                            'recommendation': 'Review engine configuration or consider alternatives'
                        })
                    ...
                    if avg_time > 0.5:  # 500ms threshold
                        alerts.append({
                            ...
                            'engine': engine_name,
                            'severity': 'medium',
                            'message': f'Engine {engine_name} avg response time: {avg_time:.3f}s',
                ...
                alerts.append({
                    ...
                    'engine': engine_name,
                    'severity': 'high',
                    'message': f'Engine {engine_name} is not responding',
                    'recommendation': 'Check engine availability and configuration'
                })
    ...
    def _get_optimization_recommendations(self) -> List[Dict[str, Any]]:
        ...
        if cache_hit_rate < 0.3:
            recommendations.append({
                ...
                'priority': 'medium',
                'message': f'Cache hit rate is low ({cache_hit_rate:.1%})',
                'action': 'Consider increasing cache size or reviewing cache key strategy'
            })
        
        # Análisis de distribución de engines
        engine_usage = self._calculate_engine_usage_distribution()
        dominant_engine = max(engine_usage.items(), key=lambda x: x[1]) if engine_usage else None
        
        if dominant_engine and dominant_engine[1] > 0.8:
            recommendations.append({
                'type': 'load_balancing',
                'priority': 'low',
                'message': f'Engine {dominant_engine[0]} handles {dominant_engine[1]:.1%} of requests',
                'action': 'Consider load balancing strategies or engine capability review'
            })
        
        # Análisis de fallos recurrentes
        frequent_failures = self._identify_frequent_failure_patterns()
        for failure_pattern in frequent_failures:
            recommendations.append({
                'type': 'failure_prevention',
                'priority': 'high',
                'message': f'Frequent failures in {failure_pattern["context"]}',
                'action': failure_pattern['recommended_action']
            })
    ...
    def generate_efficiency_report(self) -> Dict[str, Any]:
        """
        Genera reporte detallado de eficiencia del sistema.
        
        Returns:
            Reporte comprensivo con métricas de eficiencia
        """
        report = {
            'timestamp': time.time(),
            'report_period': '24h',  # Configurable
            'system_efficiency': self._calculate_system_efficiency(),
            'engine_efficiency': self._calculate_engine_efficiency(),
    ...
    def get_optimization_recommendations(self) -> List[Dict[str, Any]]:
        """Método público para obtener recomendaciones de optimización"""
    ...
    def check_performance_alerts(self) -> Dict[str, Any]:
        ...
        Returns:
            Estado de alertas y recomendaciones
        """
        alert_system = {
            'status': 'operational',
            'active_alerts': self._get_performance_alerts(),
            'alert_summary': self._summarize_alerts(),
            'monitoring_metrics': {
                'last_check': time.time(),
                'check_frequency': '5min',
                'alert_threshold_breaches': self._count_threshold_breaches()
            }
        }
        
        # Determinar estado general
        high_severity_alerts = [a for a in alert_system['active_alerts'] if a.get('severity') == 'high']
        if high_severity_alerts:
            alert_system['status'] = 'degraded'
        elif len(alert_system['active_alerts']) > 5:
            alert_system['status'] = 'warning'
    ...
    def _calculate_cache_hit_rate(self) -> float:
        ...
        total_requests = self._performance_metrics.get('cache_hits', 0) + self._performance_metrics.get('cache_misses', 0)
        if total_requests == 0:
            return 0.0
        return self._performance_metrics.get('cache_hits', 0) / total_requests
    
    def _calculate_avg_selection_time(self) -> float:
        """Calcula tiempo promedio de selección"""
        selection_times = self._performance_metrics.get('selection_times', [])
        return sum(selection_times) / len(selection_times) if selection_times else 0.0
    
    def _get_overall_health_status(self) -> str:
        """Determina estado general de salud del sistema"""
        alerts = self._get_performance_alerts()
    ...
    def _calculate_recent_avg_selection_time(self) -> float:
        """Calcula tiempo promedio de selección reciente (últimas 100 operaciones)"""
        selection_times = self._performance_metrics.get('selection_times', [])
        recent_times = selection_times[-100:] if len(selection_times) > 100 else selection_times
        return sum(recent_times) / len(recent_times) if recent_times else 0.0
    
    def _calculate_selection_time_trend(self) -> str:
        """Determina tendencia de tiempo de selección"""
        selection_times = self._performance_metrics.get('selection_times', [])
        if len(selection_times) < 20:
            return 'insufficient_data'
        
        recent = selection_times[-10:]
        older = selection_times[-20:-10]
    ...
    def _calculate_engine_usage_distribution(self) -> Dict[str, float]:
        """Calcula distribución de uso por engine"""
        operation_counts = self._performance_metrics.get('operation_counts', {})
        total = sum(operation_counts.values())
        
        if total == 0:
            return {}
        
        return {engine: count / total for engine, count in operation_counts.items()}
    
    def _identify_frequent_failure_patterns(self) -> List[Dict[str, str]]:
        """Identifica patrones de fallos frecuentes"""
        # Implementación simplificada - en producción sería más sofisticada
        patterns = []
        
        for engine_name in EngineRegistry.list_engines():
            failure_count = self.failure_registry.failure_counts.get(engine_name, 0)
            if failure_count > 5:
                patterns.append({
                    'context': f'engine_{engine_name}',
                    'recommended_action': f'Review {engine_name} configuration and dependencies'
                })
    ...
    def run_comparative_benchmark(self, operation: str, test_content: str, 
                                pattern: str, engines: List[str] = None, 
                                iterations: int = 5) -> Dict[str, Dict[str, Any]]:
        """Wrapper para benchmark comparativo"""
        if not hasattr(self, "_performance_benchmark"):
            self._performance_benchmark = PerformanceBenchmark()
        return self._performance_benchmark.run_comparative_benchmark(
            operation, test_content, pattern, engines, iterations)

    def predict_optimal_engine(self, operation: str, content: str, 
                            context: Dict[str, Any] = None) -> Dict[str, Any]:
        """
        Predice el engine óptimo usando ML simple basado en datos históricos.
        
        Args:
            operation: Tipo de operación
            content: Contenido a procesar
            context: Contexto adicional (language, complexity, etc.)
            
        Returns:
            Predicción con engine recomendado y métricas de confianza
        """
        if context is None:
            context = {}
        ...
        try:
            ...
            content_features = self._extract_content_features(content, context)
            
            # Obtener datos históricos de engines
            engine_scores = self._calculate_ml_engine_scores(operation, content_features)
            
            # Seleccionar mejor engine basado en ML
            best_engine = max(engine_scores.items(), key=lambda x: x[1]['score'])
            
            prediction = {
                'recommended_engine': best_engine[0],
                'confidence_score': best_engine[1]['score'],
                'prediction_factors': best_engine[1]['factors'],
                'alternative_engines': sorted(
                    [(k, v['score']) for k, v in engine_scores.items() if k != best_engine[0]],
            ...
            return {
                'recommended_engine': 'native',  # Fallback seguro
                'confidence_score': 0.5,
                'error': str(e),
                'prediction_failed': True
            }
    
    def _extract_content_features(self, content: str, context: Dict[str, Any]) -> Dict[str, Any]:
        """Extrae características del contenido para ML"""
        features = {
            ...
            'indentation_style': 'spaces' if '    ' in content else 'tabs' if '\t' in content else 'mixed'
        }
    ...
    def _calculate_content_complexity(self, content: str) -> float:
        """Calcula score de complejidad del contenido"""
        ...
        if 'async' in content or 'await' in content:
            complexity_factors += 1
        if any(pattern in content for pattern in ['decorator', '@', 'lambda']):
            complexity_factors += 1
        ...
        return min(complexity_factors / max_factors, 1.0)
    
    def _calculate_ml_engine_scores(self, operation: str, features: Dict[str, Any]) -> Dict[str, Dict[str, Any]]:
        """Calcula scores ML para cada engine"""
        engine_scores = {}
        
        for engine_name in ['native', 'comby', 'ast']:
            try:
                engine = EngineRegistry.get_engine(engine_name)
                
                # Score base por métricas históricas
                base_score = 0.5
                if hasattr(engine, '_metrics'):
                    success_rate = engine._metrics.get_success_rate(operation)
                    avg_time = engine._metrics.get_avg_response_time(operation)
                    base_score = success_rate * 0.7 + max(0, (1.0 - avg_time)) * 0.3
                
                # Ajustes basados en características del contenido
                feature_score = self._calculate_feature_compatibility_score(engine_name, features)
                
                # Score final combinado
                final_score = (base_score * 0.6) + (feature_score * 0.4)
                
                engine_scores[engine_name] = {
                    'score': final_score,
                    'factors': {
                        'historical_performance': base_score,
                        'feature_compatibility': feature_score,
                        'engine_capabilities': self._get_engine_capability_score(engine_name, operation)
                    }
                }
                ...
                engine_scores[engine_name] = {
                    'score': 0.1,
                    'factors': {'error': 'Engine unavailable'}
                }
        
        return engine_scores
    
    def _calculate_feature_compatibility_score(self, engine_name: str, features: Dict[str, Any]) -> float:
        """Calcula compatibilidad entre engine y características del contenido"""
        score = 0.5  # Base score
        
        # Reglas específicas por engine basadas en características
        if engine_name == 'native':
            # Native es mejor para contenido simple y operaciones básicas
            if features['complexity'] == 'low':
                score += 0.2
            if not features['has_regex_patterns']:
                score += 0.1
            if features['content_length'] < 500:
                score += 0.1
                
        elif engine_name == 'comby':
            # Comby es mejor para patrones estructurados y refactoring
            if features['has_functions'] or features['has_classes']:
                score += 0.2
            if features['language'] in ['python', 'javascript', 'java']:
                score += 0.1
            if features['complexity'] in ['medium', 'high']:
                score += 0.1
                
        elif engine_name == 'ast':
            # AST es mejor para análisis sintáctico profundo
            if features['has_functions'] and features['has_classes']:
                score += 0.3
            if features['language'] in ['python', 'javascript']:
                score += 0.2
    ...
    def _get_engine_capability_score(self, engine_name: str, operation: str) -> float:
        """Score basado en capacidades del engine para la operación"""
        try:
            engine = EngineRegistry.get_engine(engine_name)
            
            # Mapeo de operaciones a capacidades importantes
            operation_capabilities = {
                'search': [EngineCapability.LITERAL_SEARCH, EngineCapability.REGEX_SEARCH],
                'replace': [EngineCapability.LITERAL_SEARCH, EngineCapability.STRUCTURAL_SEARCH],
                'insert_before': [EngineCapability.CONTEXT_AWARE],
                'insert_after': [EngineCapability.CONTEXT_AWARE]
            }
            
            required_caps = operation_capabilities.get(operation, [])
            supported_caps = [cap for cap in required_caps if engine.supports_capability(cap)]
            
            if not required_caps:
                return 0.5
                
            return len(supported_caps) / len(required_caps)
            
        except Exception:
            return 0.3
    
    def record_operation_feedback(self, engine: str, operation: str, 
                                success: bool, execution_time: float, 
                                user_satisfaction: int = None) -> Dict[str, Any]:
        """
        Registra feedback de operación para aprendizaje adaptativo.
        
        Args:
            engine: Nombre del engine usado
            operation: Tipo de operación
            success: Si la operación fue exitosa
            execution_time: Tiempo de ejecución
            user_satisfaction: Rating 1-5 (opcional)
            
        Returns:
            Confirmación de registro y métricas actualizadas
        """
        try:
            # Registrar en métricas del engine
            engine_instance = EngineRegistry.get_engine(engine)
            if hasattr(engine_instance, '_metrics'):
                engine_instance._metrics.record_operation(
                    operation, success, execution_time
                )
            
            # Registrar feedback adicional para ML
            feedback_entry = {
                'timestamp': time.time(),
                'engine': engine,
                'operation': operation,
            ...
            # Almacenar en historial de feedback (simplificado)
            if not hasattr(self, '_ml_feedback_history'):
                self._ml_feedback_history = []
            
            self._ml_feedback_history.append(feedback_entry)
            
            # Mantener solo últimos 1000 registros
            if len(self._ml_feedback_history) > 1000:
                self._ml_feedback_history = self._ml_feedback_history[-1000:]
            
            return {
                'feedback_recorded': True,
                'engine': engine,
                'operation': operation,
                'total_feedback_entries': len(getattr(self, '_ml_feedback_history', [])),
                'updated_metrics': engine_instance._metrics.get_performance_summary() if hasattr(engine_instance, '_metrics') else {}
            }
    ...
    def auto_optimize_parameters(self) -> Dict[str, Any]:
        """
        Optimización automática de parámetros basada en datos históricos.
        
        Returns:
            Reporte de optimizaciones aplicadas
        """
        optimizations = {
            'cache_optimization': self._optimize_cache_parameters(),
            'selection_optimization': self._optimize_selection_parameters(),
            'threshold_optimization': self._optimize_threshold_parameters(),
            'optimizations_applied': 0,
            'timestamp': time.time()
        }
        
        # Contar optimizaciones exitosas
        for opt_key, opt_result in optimizations.items():
            if isinstance(opt_result, dict) and opt_result.get('applied', False):
                optimizations['optimizations_applied'] += 1
        
        return optimizations
    
    def _optimize_cache_parameters(self) -> Dict[str, Any]:
        """Optimiza parámetros de cache basado en uso histórico"""
        cache_hit_rate = self._calculate_cache_hit_rate()
        current_size = getattr(self, '_cache_max_size', 100)
        ...
        if cache_hit_rate < 0.3 and current_size < 500:
            # Aumentar tamaño de cache
            new_size = min(current_size * 2, 500)
            self._cache_max_size = new_size
            optimization.update({
                'new_value': new_size,
                'applied': True,
                'reason': f'Low cache hit rate ({cache_hit_rate:.1%})'
            })
        elif cache_hit_rate > 0.9 and current_size > 50:
            ...
            optimization.update({
                'new_value': new_size,
                'applied': True,
                'reason': f'High cache hit rate ({cache_hit_rate:.1%}), reducing memory usage'
            })
        
        return optimization
    
    def _optimize_selection_parameters(self) -> Dict[str, Any]:
        """Optimiza parámetros de selección de engines"""
        return {
            'parameter': 'selection_strategy',
            'applied': False,
            'reason': 'No optimization needed - selection parameters stable'
        }
    
    def _optimize_threshold_parameters(self) -> Dict[str, Any]:
        """Optimiza umbrales de alerta basado en historial"""
        alerts = self._get_performance_alerts()
        alert_count = len(alerts)
        
        optimization = {
            'parameter': 'alert_thresholds',
            'applied': False
        }
        
        if alert_count > 10:
            # Demasiadas alertas - relajar umbrales
            optimization.update({
                'applied': True,
                'action': 'relaxed_thresholds',
                'reason': f'Too many alerts ({alert_count}), adjusting sensitivity'
            })
        elif alert_count == 0:
            # Pocas alertas - endurecer umbrales
            optimization.update({
                'applied': True,
                'action': 'tightened_thresholds', 
                'reason': 'No alerts detected, increasing sensitivity'
            })
    ...
    def _identify_peak_performance_times(self) -> List[str]:
        """Identifica horas pico de performance"""
        # Implementación simplificada
        return ["morning", "afternoon"]  # Placeholder para análisis temporal real
    
    def _calculate_engine_usage_trends(self) -> Dict[str, str]:
        """Calcula tendencias de uso por engine"""
        return {"native": "stable", "comby": "increasing", "ast": "stable"}
    
    def _calculate_error_rate_trends(self) -> Dict[str, float]:
        """Calcula tendencias de tasa de error"""
        return {"overall": 0.05, "trend": "stable"}
    ...
    def _calculate_engine_efficiency(self) -> Dict[str, float]:
        """Calcula eficiencia por engine"""
        return {
            "native": 0.90,
            "comby": 0.75,
            "ast": 0.80
    ...
    def _calculate_resource_utilization(self) -> Dict[str, float]:
        ...
        return {
            "cpu": 0.45,
            "memory": 0.60,
            "cache": self._calculate_cache_hit_rate()
        }
    ...
    def _identify_bottlenecks(self) -> List[Dict[str, str]]:
        ...
        avg_time = self._calculate_avg_selection_time()
        if avg_time > 0.1:  # 100ms threshold
            bottlenecks.append({
                "type": "selection_time",
                "description": f"High selection time: {avg_time:.3f}s",
                "severity": "medium"
            })
    ...
    def _count_threshold_breaches(self) -> int:
        """Cuenta violaciones de umbrales"""
        return len(self._get_performance_alerts())
