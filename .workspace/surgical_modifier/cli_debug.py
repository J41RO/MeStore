...
try:
    from core.operations.basic.replace import (
        replace_content, replace_content_v53, replace_operation,
        replace_content_regex, ReplaceOperation
    )
...
try:
    from core.operations.basic.after import (
        insert_after, insert_after_v53, after_operation,
        insert_after_regex, AfterOperation
    )
...
try:
    from core.operations.basic.before import (
        insert_before, insert_before_v53, before_operation,
        insert_before_regex, BeforeOperation
    )
...
try:
    from core.operations.basic.append import (
        append_content, append_content_v53, append_operation,
        append_with_separator, AppendOperation
    )
...
def create(filepath, content, template, variables, backup, verbose):
    ...
    try:
        if template:
            ...
            print(f"DEBUG: Llamando create_file({filepath}, {content})")
                success = create_file(filepath, content)
            print(f"DEBUG: create_file retornó: {success}")
...
@click.option('--case-sensitive/--ignore-case', default=True, help='Case sensitive matching')
@click.option('--max-replacements', type=int, default=-1, help='Maximum replacements (-1 for all)')
@click.option('--verbose', '-V', is_flag=True, help='Show detailed output')
def replace(filepath, pattern, replacement, regex, case_sensitive, max_replacements, verbose):
    ...
    try:
        if regex:
            result = replace_content_regex(filepath, pattern, replacement, 
                                         case_sensitive=case_sensitive, 
                                         max_replacements=max_replacements)
        else:
            result = replace_content(filepath, pattern, replacement,
                                   case_sensitive=case_sensitive,
                                   max_replacements=max_replacements)
        
        if hasattr(result, 'success') and result.success:
            replacements = result.details.get('replacements_made', 0)
            console.print(f"✅ Successfully replaced {replacements} occurrence(s) in {filepath}", style="green")
            if verbose:
...
@click.option('--regex/--no-regex', default=False, help='Use regex pattern matching')
@click.option('--case-sensitive/--ignore-case', default=True, help='Case sensitive matching')
@click.option('--first-match/--all-matches', default=True, help='Insert after first match only')
@click.option('--preserve-indent/--no-indent', default=True, help='Preserve indentation')
@click.option('--verbose', '-V', is_flag=True, help='Show detailed output')
def after(filepath, pattern, content, regex, case_sensitive, first_match, preserve_indent, verbose):
    ...
    try:
        if regex:
            result = insert_after_regex(filepath, pattern, content,
                                      case_sensitive=case_sensitive,
                                      first_match_only=first_match,
                                      preserve_indentation=preserve_indent)
        else:
            result = insert_after(filepath, pattern, content,
                                case_sensitive=case_sensitive,
                                first_match_only=first_match,
                                preserve_indentation=preserve_indent)
        ...
        if hasattr(result, 'success') and result.success:
            ...
            console.print(f"✅ Successfully inserted content after {insertions} occurrence(s) in {filepath}", style="green")
            if verbose:
                console.print(f"📁 File: {filepath}")
                console.print(f"🔍 Pattern: {pattern}")
...
@click.option('--regex/--no-regex', default=False, help='Use regex pattern matching')
@click.option('--case-sensitive/--ignore-case', default=True, help='Case sensitive matching')
@click.option('--first-match/--all-matches', default=True, help='Insert before first match only')
@click.option('--preserve-indent/--no-indent', default=True, help='Preserve indentation')
@click.option('--verbose', '-V', is_flag=True, help='Show detailed output')
def before(filepath, pattern, content, regex, case_sensitive, first_match, preserve_indent, verbose):
    ...
    try:
        if regex:
            result = insert_before_regex(filepath, pattern, content,
                                       case_sensitive=case_sensitive,
                                       first_match_only=first_match,
                                       preserve_indentation=preserve_indent)
        else:
            result = insert_before(filepath, pattern, content,
                                 case_sensitive=case_sensitive,
                                 first_match_only=first_match,
                                 preserve_indentation=preserve_indent)
        ...
        if hasattr(result, 'success') and result.success:
            insertions = result.details.get('insertions_made', 0)
            console.print(f"✅ Successfully inserted content before {insertions} occurrence(s) in {filepath}", style="green")
            if verbose:
                console.print(f"📁 File: {filepath}")
                console.print(f"🔍 Pattern: {pattern}")
                console.print(f"➕ Content: {content}")
...
@click.argument('filepath', type=str)
@click.argument('content', type=str)
@click.option('--add-newline/--no-newline', default=True, help='Add newline before content')
@click.option('--separator', type=str, default='', help='Separator to add before content')
@click.option('--verbose', '-V', is_flag=True, help='Show detailed output')
def append(filepath, content, add_newline, separator, verbose):
    ...
    try:
        ...
        if hasattr(result, 'success') and result.success:
            ...
            console.print(f"✅ Successfully appended {chars_added} characters to {filepath}", style="green")
            if verbose:
                console.print(f"📁 File: {filepath}")
                console.print(f"➕ Content: {content}")
                console.print(f"🔗 Separator: {repr(separator) if separator else 'None'}")
...
{chr(10).join(f'  {ex}' for ex in info['examples'])}
    ...
    if RICH_AVAILABLE:
        console.print(Panel(info_text.strip(), title=f"Operation Info: {operation_name.upper()}", border_style="blue"))
    else:
        print(info_text)
