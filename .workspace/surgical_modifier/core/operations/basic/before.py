...
class BeforeOperation(BaseOperation):
   ...
   def execute(self, context: OperationContext) -> OperationResult:
       ...
       try:
           ...
           first_match_only = context.arguments.get('first_match_only', True) if context.arguments else True
           preserve_indentation = context.arguments.get('preserve_indentation', True) if context.arguments else True
           
           # Perform insertion
           insertion_result = self._perform_before_insertion(
               original_content, pattern, content,
               regex_mode, case_sensitive, first_match_only, preserve_indentation
           )
   ...
   def _perform_before_insertion(self, content: str, pattern: str, insertion: str,
                                regex_mode: bool, case_sensitive: bool, 
                                first_match_only: bool, preserve_indentation: bool) -> dict:
       """
       ...
       try:
           ...
           for i, line in enumerate(lines):
               ...
               if self._line_matches_pattern(line, pattern, regex_mode, case_sensitive):
                   ...
                   if preserve_indentation:
                       ...
                       content_to_insert = self._apply_indentation(insertion, indentation)
   ...
   def _line_matches_pattern(self, line: str, pattern: str, regex_mode: bool, case_sensitive: bool) -> bool:
   ...
   def insert_before_v53(self, file_path: str, pattern: str, content: str, 
       ...
       try:
           ...
           if target_str not in self.inserted_content:
               if LOGGING_AVAILABLE and logger:
                   logger.warning(f"No insertion record found for rollback: {target_file}")
               return False
...
def insert_before(target_file: str, pattern: str, content: str, **kwargs) -> OperationResult:
   """
...
def insert_before_regex(target_file: str, pattern: str, content: str, **kwargs) -> OperationResult:
   """
...
def insert_before_v53(file_path: str, pattern: str, content: str, 
                     regex_mode: bool = False, **kwargs) -> OperationResult:
   """
   ...
   Returns:
       OperationResult with v5.3 compatibility
   """
   return before_operation.insert_before_v53(file_path, pattern, content, regex_mode, **kwargs)