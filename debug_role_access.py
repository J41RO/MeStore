...
def debug_user_role_data():
    ...
    try:
        ...
        print(f"   Valor en role_hierarchy: {role_hierarchy.get(user_type, 'NO ENCONTRADO')}")
        ...
        print(f"   ¿Acceso permitido? {'✅ SÍ' if user_level >= required_level else '❌ NO'}")
        
        # Test de comparación exacta
        print(f"\n🎯 TEST DE COMPARACIÓN EXACTA:")
        print(f"   user_type == 'VENDEDOR': {user_type == 'VENDEDOR'}")
        print(f"   user_type === 'VENDEDOR' (JS equivalent): {user_type is not None and str(user_type) == 'VENDEDOR'}")
        
        # Verificar espacios o caracteres ocultos
        if user_type != user_type.strip():
            print(f"   ⚠️ PROBLEMA: user_type tiene espacios! '{user_type}' vs '{user_type.strip()}'")
