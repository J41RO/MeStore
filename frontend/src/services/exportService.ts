...
export interface ExportRequest {
  tipo_reporte: 'resumen' | 'ventas' | 'productos_top' | 'comisiones' | 'inventario' | 'completo';
  formato: 'pdf' | 'excel';
  fecha_inicio?: string;
  fecha_fin?: string;
  vendedor_id?: number;
...
class ExportService {
  ...
  async exportReport(request: ExportRequest): Promise<ExportResponse> {
    try {
      ...
      if (request.fecha_inicio) params.append('fecha_inicio', request.fecha_inicio);
      if (request.fecha_fin) params.append('fecha_fin', request.fecha_fin);
      if (request.vendedor_id) params.append('vendedor_id', request.vendedor_id.toString());

      const response = await axios.get(`${this.baseURL}/dashboard/exportar?${params.toString()}`, {
  ...
  async exportAndDownload(request: ExportRequest): Promise<ExportResponse> {
    ...
    if (exportResult.success && exportResult.file_url && exportResult.filename) {
