...
interface UserContextType extends UserContextState {
  ...
  updateSettings: (
    settings: Partial<VendorProfile['settings']>
  ) => Promise<boolean>;
  updateContactInfo: (
    contactInfo: Partial<VendorProfile['contactInfo']>
  ) => Promise<boolean>;
...
export const UserProvider: React.FC<UserProviderProps> = ({ children }) => {
  ...
  const generateMockVendorProfile = (
    userId: string,
    email: string,
    name?: string
  ): VendorProfile => {
  ...
  const loadVendorProfile = async (): Promise<void> => {
    ...
    try {
      ...
      const mockProfile = generateMockVendorProfile(
        user.id,
        user.email,
        user.name
      );
      mockProfile.profileStatus.completionPercentage =
  ...
  const updateVendorProfile = async (
    ...
    try {
      ...
      updatedProfile.profileStatus.completionPercentage =
  ...
  const refreshMetrics = async (): Promise<void> => {
    ...
    try {
      ...
      const updatedMetrics = {
        ...
        lastActivity: new Date().toISOString(),
        totalSales:
          vendorProfile.businessMetrics.totalSales +
  ...
  const getProfileMissingFields = (): string[] => {
    ...
    if (
      vendorProfile.contactInfo.phone === undefined ||
      vendorProfile.contactInfo.phone === ''
    )
      missingFields.push('Teléfono');
    if (
      vendorProfile.contactInfo.address === undefined ||
      vendorProfile.contactInfo.address === ''
    )
      missingFields.push('Dirección');
    if (
      vendorProfile.contactInfo.city === undefined ||
      vendorProfile.contactInfo.city === ''
    )
      missingFields.push('Ciudad');
    if (
      vendorProfile.storeDescription === undefined ||
      vendorProfile.storeDescription.length < 10
    ) {
  ...
  const completionPercentage =
    vendorProfile?.profileStatus.completionPercentage || 0;
