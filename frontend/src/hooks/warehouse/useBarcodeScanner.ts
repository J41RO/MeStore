...
import { InventoryStatus, LocationZone } from '../../types/inventory.types';
...
export const useBarcodeScanner = (
  ...
  const [currentSession, setCurrentSession] = useState<PickingSession | null>(
    null
  );
  ...
  const scanItem = useCallback(
    async (sku: string): Promise<ScannedItem | null> => {
      setIsScanning(true);
      ...
      try {
        await new Promise(resolve =>
          setTimeout(resolve, scannerConfig.simulationDelay)
        );
        ...
        if (inventoryItem) {
          const item: ScannedItem = {
            ...
            quantity: 1,
            location:
              inventoryItem.location.zone + '-' + inventoryItem.location.aisle,
            timestamp: new Date(),
  ...
  const startPickingSession = useCallback(
    (sessionConfig?: Partial<PickingSession>) => {
  ...
  return {
    ...
    stats: {
      ...
      successfulScans: scannedItems.filter(
        item => item.status === ScanStatus.SUCCESS
      ).length,
      errorScans: scannedItems.filter(item => item.status === ScanStatus.ERROR)
        .length,
    },
  };
};
