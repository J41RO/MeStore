...
describe('ResetPassword Component', () => {
  ...
  test('renders loading screen while validating token', () => {
    ...
    expect(screen.getByText('Validando enlace de recuperación...')).toBeInTheDocument();
  ...
  test('shows error screen when no token in URL', async () => {
    ...
    expect(screen.getByText('Token no encontrado en la URL')).toBeInTheDocument();
  });
  ...
  test('handles password reset API error', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('Token expirado durante el reset')).toBeInTheDocument();
    });
