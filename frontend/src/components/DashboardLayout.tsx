...
const DashboardLayout: React.FC<DashboardLayoutProps> = ({ children }) => {
      ...
      {navigationItems.map((item) => {
        ...
        return (
          <a 
            key={item.name}
            href={item.href} 
            className={isActive ? "block p-2 bg-blue-100 text-blue-700 rounded mb-1" : "block p-2 text-gray-600 hover:bg-gray-100 rounded mb-1"}
            onClick={(e) => {
              e.preventDefault();
              navigate(item.href);
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex">
      ...
      {/* Mobile sidebar */}
      <div className={sidebarOpen ? "fixed inset-y-0 left-0 z-50 w-64 bg-white shadow-lg transform translate-x-0 transition-transform lg:hidden" : "fixed inset-y-0 left-0 z-50 w-64 bg-white shadow-lg transform -translate-x-full transition-transform lg:hidden"}>
        <div className="h-16 bg-blue-600 flex items-center justify-center">
          <h1 className="text-white font-bold">MeStore</h1>
        </div>
        <nav className="p-4">
