  ...
  useEffect(() => {
    const fetchHistorial = async () => {
      try {
        ...
        const response = await fetch(`/api/v1/comisiones/payout-history/${payoutId}`, {
          headers: {
      ...
      <div className={`p-4 ${className}`}>
        <div className="bg-red-50 border border-red-200 rounded-md p-4">
          <div className="flex">
            ...
            <div className="ml-3">
              <h3 className="text-sm font-medium text-red-800">Error al cargar historial</h3>
              ...
              {historial.map((entry, index) => (
                <tr key={entry.id} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    {entry.estado_anterior ? getEstadoBadge(entry.estado_anterior) : (
                      <span className="text-gray-400 italic">Estado inicial</span>
                    )}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
