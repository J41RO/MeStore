...
import { AlertsPanelProps, AlertsPanelAlert, AlertType, AlertCategory } from '../../types/alerts.types';
import { Bell, Package, RefreshCw } from 'lucide-react';

const AlertsPanel: React.FC<AlertsPanelProps> = ({ className = '', maxAlerts = 50, onAlertClick }) => {
  ...
  const getSeverityClass = (alert: AlertsPanelAlert) => {
    ...
    const severity = alert.severity === 'critical' ? 'border-l-red-500 bg-red-50' :
                    alert.severity === 'high' ? 'border-l-orange-500 bg-orange-50' :
                    alert.severity === 'medium' ? 'border-l-yellow-500 bg-yellow-50' :
                    'border-l-blue-500 bg-blue-50';
    const opacity = !alert.isRead ? ' shadow-sm' : ' opacity-75';
            ...
            <div key={alert.id} onClick={() => handleAlertClick(alert)} className={getSeverityClass(alert)}>
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-gray-600 mt-1">{getAlertContent(alert)}</p>
                  
                  <div className="flex items-center gap-4 mt-2 text-xs text-gray-500">
                    <span>{alert.timestamp.toLocaleTimeString()}</span>
                    {alert.actionRequired && (
                      <span className="px-2 py-1 bg-red-100 text-red-700 rounded-full">Acción requerida</span>
                    )}
                  </div>
                </div>
