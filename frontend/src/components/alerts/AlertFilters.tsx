...
import { AlertsFilter, AlertType, AlertSeverity, AlertCategory } from '../../types/alerts.types';
...
const AlertFilters: React.FC<AlertFiltersProps> = ({
  ...
  return (
    <div className="absolute top-12 right-0 z-50 w-80 bg-white border border-gray-200 rounded-lg shadow-lg p-4">
      ...
      {/* Tipos de Alerta */}
      <div className="mb-4">
        <h4 className="text-sm font-medium text-gray-700 mb-2">Tipo de Alerta</h4>
        <div className="space-y-2">
          {Object.values(AlertType).map(type => (
            <label key={type} className="flex items-center gap-2 cursor-pointer">
              ...
              {filters.types.includes(type) && <Check className="h-3 w-3 text-green-500" />}
            </label>
          ))}
        </div>
      </div>
        ...
        <div className="space-y-2">
          ...
          <label className="flex items-center gap-2 cursor-pointer">
            <input
              ...
              onChange={(e) => onFiltersChange({ ...filters, showRead: e.target.checked })}
              className="rounded border-gray-300 text-blue-600 focus:ring-blue-500"
            />
