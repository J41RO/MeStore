...
export const BarcodeScannerWithPicking: React.FC<
  BarcodeScannerWithPickingProps
> = ({ mode = ScanMode.MANUAL, className = '' }) => {
  const {
    currentSession,
    scannedItems,
    startPickingSession,
  ...
  return (
    <div className={`space-y-6 ${className}`}>
          ...
          <BarcodeScanner
            ...
            placeholder={
              currentSession
                ...
                : 'Inicia una sesión para escanear'
            }
          />
        </div>
      ...
      <div className='text-center text-sm text-gray-500'>
        ...
        {currentSession && (
          <span className='ml-4'>Sesión: {currentSession.id}</span>
        )}
      </div>
    </div>
  );
