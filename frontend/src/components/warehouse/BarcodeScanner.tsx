...
export const BarcodeScanner: React.FC<BarcodeScannerProps> = ({
  ...
  const getScannerStyle = () => {
    const baseStyle = 'relative border-2 rounded-lg p-4 transition-all duration-300';
    ...
    <div className={`${getScannerStyle()} ${className}`}>
        ...
        <div className="mb-4 p-4 bg-blue-50 border border-blue-200 rounded-lg">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1">
              <div className="text-sm font-medium text-blue-900">Escaneando código...</div>
      ...
      {lastScan && (
        <div className="mb-4 p-4 bg-green-50 border border-green-200 rounded-lg">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <div className="font-semibold text-green-900 mb-1">{lastScan.name}</div>
              <div className="grid grid-cols-2 gap-2 text-sm">
                ...
                <div>
                  ...
                  <span className="ml-1 text-green-600 font-mono text-xs">{lastScan.barcode}</span>
                </div>
                <div>
                  <span className="font-medium text-green-700">Ubicación:</span>
                  <span className="ml-1 text-green-600">{lastScan.location}</span>
                </div>
                <div>
                  <span className="font-medium text-green-700">Escaneado:</span>
                  <span className="ml-1 text-green-600">{lastScan.timestamp.toLocaleTimeString()}</span>
                </div>
              </div>
      ...
      {stats.totalScans > 0 && (
        <div className="mt-4 p-3 bg-gray-50 border border-gray-200 rounded-lg">
          <div className="text-xs font-medium text-gray-700 mb-2">Estadísticas de Sesión</div>
          <div className="grid grid-cols-3 gap-4 text-center">
            <div>
              <div className="text-lg font-bold text-gray-900">{stats.totalScans}</div>
              <div className="text-xs text-gray-600">Total</div>
            </div>
            <div>
              <div className="text-lg font-bold text-green-600">{stats.successfulScans}</div>
              <div className="text-xs text-gray-600">Exitosos</div>
            </div>
            <div>
              <div className="text-lg font-bold text-red-600">{stats.errorScans}</div>
