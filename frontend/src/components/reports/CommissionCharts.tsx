...
const CommissionCharts: React.FC<CommissionChartsProps> = ({ breakdown }) => {
  ...
  const categoryData = Object.entries(breakdown.byCategory).map(([category, amount]) => ({
    name: category,
    amount: amount
  }));
  ...
  return (
    <div className="space-y-8">
      <div>
        <h3 className="text-lg font-medium text-gray-900 mb-4">Comisiones por Categoría</h3>
        <div className="h-80 w-full">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={categoryData}>
              ...
              />
              <YAxis />
              <Tooltip formatter={(value) => [`$${Number(value).toFixed(2)}`, 'Comisiones']} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div>
          <h3 className="text-lg font-medium text-gray-900 mb-4">Tendencia por Período</h3>
          <div className="h-64 w-full">
            <ResponsiveContainer width="100%" height="100%">
      ...
      <div className="bg-gray-50 p-4 rounded-lg">
        <h4 className="text-md font-medium text-gray-900 mb-2">Resumen de Gráficos</h4>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
          <div>
            <p className="text-gray-600">Categoría Principal:</p>
            <p className="font-semibold">{breakdown.totals.topCategory || 'N/A'}</p>
          </div>
          <div>
            <p className="text-gray-600">Producto Principal:</p>
            <p className="font-semibold">{breakdown.totals.topProduct || 'N/A'}</p>
          </div>
          <div>
            <p className="text-gray-600">Tasa Promedio:</p>
            <p className="font-semibold">{(breakdown.totals.averageCommissionRate * 100).toFixed(2)}%</p>
          </div>
        </div>
      </div>
    </div>
