...
const CommissionSummary: React.FC<CommissionSummaryProps> = ({
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="bg-gradient-to-r from-purple-500 to-purple-600 p-6 rounded-lg text-white">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold">{(breakdown.totals.averageCommissionRate * 100).toFixed(2)}%</p>
            </div>
        ...
        <div className="bg-gradient-to-r from-orange-500 to-orange-600 p-6 rounded-lg text-white">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold">{breakdown.totals.commissionCount}</p>
            </div>
            ...
            {topProducts.map((product, index) => (
              <div key={product.productId} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                ...
                <div className="text-right">
                  <p className="text-sm font-medium text-green-600">${product.totalCommissions.toFixed(2)}</p>
                  <p className="text-xs text-gray-500">{product.commissionCount} ventas</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="bg-white p-6 rounded-lg border border-gray-200">
          ...
          <div className="space-y-3">
            {topCategories.map(([category, amount], index) => {
              const percentage = totalCommissions > 0 ? (amount / totalCommissions) * 100 : 0;
              return (
      ...
      <div className="bg-white p-6 rounded-lg border border-gray-200">
        <h3 className="text-lg font-semibold text-gray-900 mb-4">Breakdown por Tipo de Comisión</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
          {breakdown.byType.map((type) => (
            <div key={type.type} className="text-center p-4 bg-gray-50 rounded-lg">
              ...
              <p className="text-2xl font-bold text-blue-600">${type.totalCommissions.toFixed(2)}</p>
              <p className="text-sm text-gray-500">{type.commissionCount} registros</p>
              <p className="text-xs text-gray-400">{type.percentage.toFixed(1)}% del total</p>
            </div>
          ))}
        </div>
      </div>
