      ...
      >
        ...
        {showStatistics && (
          <div className="flex items-center gap-3 text-xs text-gray-600 mb-1">
            <span>Ocupación: {Math.round((zoneItems.length / (config.rows * Math.floor(config.cols/2))) * 100)}%</span>
            <span>•</span>
            <span className="text-green-600">{zoneItems.filter(i => i.status === 'IN_STOCK').length} En Stock</span>
            <span className="text-yellow-600">{zoneItems.filter(i => i.status === 'LOW_STOCK').length} Bajo</span>
            <span className="text-red-600">{zoneItems.filter(i => i.status === 'OUT_OF_STOCK').length} Agotado</span>
          </div>
        )}
          ...
          {Array.from({ length: config.rows }, (_, aisleIdx) => (
            <div key={`aisle-${aisleIdx}`} className="flex items-center gap-1">
              ...
              <div className="flex gap-1">
                {Array.from({ length: Math.floor(config.cols/2) }, (_, shelfIdx) => {
                  const position = `${String.fromCharCode(65 + aisleIdx)}-${shelfIdx + 1}`;
                  const item = zoneItems.find(item => 
                    item.location.aisle === String.fromCharCode(65 + aisleIdx) && 
                  ...
                  return (
                    <div
                      ...
                      className={`
                        ...
                        ${item ? 
                          item.status === 'IN_STOCK' ? 'bg-green-100 border-green-300' :
                          item.status === 'LOW_STOCK' ? 'bg-yellow-100 border-yellow-300' :
                    ...
                    >
                      {item ? (
                        ...
                        <span className="text-gray-400 text-xs">{shelfIdx + 1}</span>
                      )}
                    </div>
                  );
                })}
    ...
    <div className={`p-4 ${className}`}>
      ...
      {selectedLocation && (
        <div className="mt-4 p-4 bg-white border-2 border-blue-200 rounded-lg shadow-sm">
          <div className="flex items-center justify-between mb-2">
            <h4 className="font-semibold text-gray-800">Información de Ubicación</h4>
            <button 
