...
class TestWompiServiceTransactionStatus:
    ...
    def mock_wompi_service(self):
        ...
        with patch.dict(os.environ, {
            ...
            "WOMPI_WEBHOOK_SECRET": "test-webhook-secret",
            "WOMPI_TIMEOUT": "30.0",
            "WOMPI_POOL_SIZE": "10",
            "WOMPI_RATE_LIMIT": "100",
            "WOMPI_RATE_WINDOW": "60",
            "TESTING": "1"
        }):
            # Create real WompiService instance with proper config
            service = WompiService()
            # Replace the HTTP client with a mock to prevent real API calls
...
class TestWompiServiceHealthCheck:
    ...
    def mock_wompi_service(self):
        ...
        with patch.dict(os.environ, {
            ...
            # Create real WompiService instance with proper config
            service = WompiService()
            # Override request count for health check tests
    ...
    async def test_health_check_invalid_configuration(self, mock_wompi_service):
        ...
        # Should be unhealthy due to configuration issues
    ...
    async def test_health_check_payment_methods_failure(self, mock_wompi_service):
        ...
        # Should be unhealthy due to payment methods failure
        assert result["status"] == "unhealthy"
...
class TestWompiServiceIntegration:
    ...
    async def test_all_methods_work_together(self, mock_wompi_service):
        ...
        mock_responses = [
            Mock(json=lambda: mock_merchant_data, headers={}),  # For health_check's merchant call
            Mock(json=lambda: mock_merchant_data, headers={}),  # For health_check's get_payment_methods call
            Mock(json=lambda: mock_merchant_data, headers={}),  # For explicit get_payment_methods
