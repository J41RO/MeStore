...
async def test_integrated_payment_service_uses_wompi_methods():
    ...
    # In testing mode, the service returns simulated health status without calling wompi_service
    health_status = await service.health_check()
    assert "wompi" in health_status["components"]
    assert health_status["components"]["wompi"]["status"] == "healthy"
    # Don't check mock calls in testing mode since service simulates health check
