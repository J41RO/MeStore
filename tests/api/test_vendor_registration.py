...
async def test_register_vendor_email_case_insensitive(async_client: AsyncClient):
    ...
    # El sistema debe rechazar el email duplicado (case-insensitive)
    assert response2.status_code == 400, f"Expected 400 for duplicate email (case-insensitive), got {response2.status_code}"

    error_data = response2.json()
    error_message = error_data.get("detail", "").lower()
    assert "email" in error_message and ("registrado" in error_message or "duplicate" in error_message), \
        f"Error message should mention email duplicate: {error_data}"
