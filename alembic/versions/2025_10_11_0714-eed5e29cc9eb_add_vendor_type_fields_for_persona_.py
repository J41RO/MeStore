"""add vendor type fields for persona natural and juridica

Revision ID: eed5e29cc9eb
Revises: eff21666914a
Create Date: 2025-10-11 07:14:39.056611+00:00
...
revision: str = 'eed5e29cc9eb'
down_revision: Union[str, Sequence[str], None] = 'eff21666914a'
...
def upgrade() -> None:
    ...
    op.alter_column('admin_permissions', 'resource_type',
               ...
               type_=sa.Enum('USERS', 'VENDORS', 'ORDERS', 'PRODUCTS', 'INVENTORY', 'TRANSACTIONS', 'COMMISSIONS', 'REPORTS', 'SETTINGS', 'AUDIT_LOGS', 'NOTIFICATIONS', 'PAYMENTS', 'STORAGE', 'ANALYTICS', 'PERMISSIONS', name='resourcetype'),
               existing_nullable=False)
    op.alter_column('admin_permissions', 'action',
               existing_type=sa.VARCHAR(length=9),
               type_=sa.Enum('CREATE', 'READ', 'UPDATE', 'DELETE', 'APPROVE', 'REJECT', 'MANAGE', 'AUDIT', 'EXPORT', 'IMPORT', 'CONFIGURE', 'MONITOR', 'GRANT', 'REVOKE', name='permissionaction'),
               existing_nullable=False)
    ...
    op.alter_column('orders', 'estimated_delivery',
               ...
               type_=sa.DateTime(timezone=True),
               existing_nullable=True)
    op.alter_column('orders', 'shipping_events',
               existing_type=sqlite.JSON(),
               type_=sa.JSON(),
               existing_nullable=True)
    ...
    op.alter_column('products', 'status',
               existing_type=sa.VARCHAR(length=10),
               type_=sa.Enum('DRAFT', 'PENDING', 'APPROVED', 'REJECTED', 'TRANSITO', 'VERIFICADO', 'DISPONIBLE', 'VENDIDO', 'INACTIVE', name='productstatus'),
               existing_nullable=False)
    ...
    op.alter_column('transactions', 'deleted_at',
               ...
               type_=sa.DateTime(),
               existing_nullable=True)
    op.add_column('users', sa.Column('google_id', sa.String(length=100), nullable=True, comment='ID único de Google para OAuth'))
    op.add_column('users', sa.Column('google_email', sa.String(length=255), nullable=True, comment='Email desde Google OAuth (puede diferir del email principal)'))
    op.add_column('users', sa.Column('google_name', sa.String(length=200), nullable=True, comment='Nombre completo desde Google OAuth'))
    op.add_column('users', sa.Column('google_picture', sa.String(length=500), nullable=True, comment='URL de foto de perfil desde Google'))
    op.add_column('users', sa.Column('google_verified_email', sa.Boolean(), nullable=False, comment='Si el email está verificado en Google'))
    op.add_column('users', sa.Column('oauth_provider', sa.String(length=50), nullable=True, comment='Proveedor OAuth usado (google, facebook, etc.)'))
    op.add_column('users', sa.Column('oauth_linked_at', sa.DateTime(timezone=True), nullable=True, comment='Fecha cuando se vinculó la cuenta OAuth'))
    op.add_column('users', sa.Column('permissions', sa.JSON(), nullable=True, comment='Lista de permisos granulares asignados al usuario. OWNER tiene todos los permisos siempre.'))
    op.add_column('users', sa.Column('security_clearance_level', sa.Integer(), nullable=False, comment='Nivel de autorización de seguridad (1-5) para usuarios admin'))
    op.add_column('users', sa.Column('department_id', sa.String(length=100), nullable=True, comment='ID del departamento al que pertenece el usuario admin'))
    op.add_column('users', sa.Column('employee_id', sa.String(length=50), nullable=True, comment='ID de empleado para usuarios administrativos'))
    op.add_column('users', sa.Column('performance_score', sa.Integer(), nullable=False, comment='Puntuación de desempeño del usuario admin (0-100)'))
    op.add_column('users', sa.Column('failed_login_attempts', sa.Integer(), nullable=False, comment='Número de intentos fallidos de login'))
    op.add_column('users', sa.Column('account_locked_until', sa.DateTime(timezone=True), nullable=True, comment='Fecha hasta la cual la cuenta está bloqueada'))
    op.add_column('users', sa.Column('force_password_change', sa.Boolean(), nullable=False, comment='Indica si el usuario debe cambiar su contraseña en el próximo login'))
    ...
    op.alter_column('users', 'user_type',
               existing_type=sa.VARCHAR(length=9),
               type_=sa.Enum('CUSTOMER', 'BUYER', 'VENDOR', 'ADMIN_MARKETING', 'ADMIN_LOGISTICS', 'ADMIN_SUPPORT', 'ADMIN_SALES', 'ADMIN', 'SUPERUSER', 'OWNER', 'SYSTEM', name='usertype'),
               existing_nullable=False)
    ...
    op.alter_column('users', 'account_status',
               ...
               type_=sa.Enum('PENDING', 'ACTIVE', 'SUSPENDED', 'DELETED', name='accountstatus'),
               existing_nullable=False)
    ...
    op.alter_column('users', 'deleted_at',
               ...
               type_=sa.DateTime(),
               existing_nullable=True)
    op.create_index('ix_user_google_id', 'users', ['google_id'], unique=False)
    op.create_index('ix_user_oauth_provider', 'users', ['oauth_provider'], unique=False)
...
def downgrade() -> None:
    ...
    op.drop_index('ix_user_oauth_provider', table_name='users')
    ...
    op.alter_column('users', 'is_active',
               ...
               existing_nullable=False)
    op.alter_column('users', 'account_status',
               existing_type=sa.Enum('PENDING', 'ACTIVE', 'SUSPENDED', 'DELETED', name='accountstatus'),
    ...
    op.alter_column('users', 'vendor_status',
               ...
               existing_nullable=True)
    op.alter_column('users', 'user_type',
               existing_type=sa.Enum('CUSTOMER', 'BUYER', 'VENDOR', 'ADMIN_MARKETING', 'ADMIN_LOGISTICS', 'ADMIN_SUPPORT', 'ADMIN_SALES', 'ADMIN', 'SUPERUSER', 'OWNER', 'SYSTEM', name='usertype'),
               type_=sa.VARCHAR(length=9),
               existing_nullable=False)
    op.alter_column('users', 'apellido',
               existing_type=sa.String(length=100),
    ...
    op.drop_column('users', 'security_clearance_level')
    ...
    op.alter_column('products', 'precio_venta',
               ...
               existing_nullable=True)
    op.alter_column('products', 'status',
               existing_type=sa.Enum('DRAFT', 'PENDING', 'APPROVED', 'REJECTED', 'TRANSITO', 'VERIFICADO', 'DISPONIBLE', 'VENDIDO', 'INACTIVE', name='productstatus'),
               type_=sa.VARCHAR(length=10),
               existing_nullable=False)
    op.alter_column('products', 'description',
               existing_type=sa.Text(),
    ...
    op.alter_column('admin_permissions', 'scope',
               ...
               existing_nullable=False)
    op.alter_column('admin_permissions', 'action',
               existing_type=sa.Enum('CREATE', 'READ', 'UPDATE', 'DELETE', 'APPROVE', 'REJECT', 'MANAGE', 'AUDIT', 'EXPORT', 'IMPORT', 'CONFIGURE', 'MONITOR', 'GRANT', 'REVOKE', name='permissionaction'),
               type_=sa.VARCHAR(length=9),
               existing_nullable=False)
    op.alter_column('admin_permissions', 'resource_type',
               existing_type=sa.Enum('USERS', 'VENDORS', 'ORDERS', 'PRODUCTS', 'INVENTORY', 'TRANSACTIONS', 'COMMISSIONS', 'REPORTS', 'SETTINGS', 'AUDIT_LOGS', 'NOTIFICATIONS', 'PAYMENTS', 'STORAGE', 'ANALYTICS', 'PERMISSIONS', name='resourcetype'),
               type_=sa.VARCHAR(length=13),
               existing_nullable=False)
    op.alter_column('admin_permissions', 'description',
               existing_type=sa.Text(),
    ...
    op.alter_column('admin_activity_logs', 'id',
               ...
               type_=sa.VARCHAR(length=36),
               existing_nullable=False)
    op.create_table('id_mapping_payment_methods',
    sa.Column('old_id', sa.INTEGER(), nullable=False),
    sa.Column('new_id', sa.VARCHAR(length=36), nullable=False),
    sa.PrimaryKeyConstraint('old_id'),
    sa.UniqueConstraint('new_id')
    )
    op.create_table('id_mapping_order_transactions',
    sa.Column('old_id', sa.INTEGER(), nullable=False),
    sa.Column('new_id', sa.VARCHAR(length=36), nullable=False),
    sa.PrimaryKeyConstraint('old_id'),
    sa.UniqueConstraint('new_id')
    )
    op.create_table('id_mapping_orders',
    sa.Column('old_id', sa.INTEGER(), nullable=False),
    sa.Column('new_id', sa.VARCHAR(length=36), nullable=False),
    sa.PrimaryKeyConstraint('old_id'),
    sa.UniqueConstraint('new_id')
    )
    op.create_table('id_mapping_order_items',
    sa.Column('old_id', sa.INTEGER(), nullable=False),
    sa.Column('new_id', sa.VARCHAR(length=36), nullable=False),
    sa.PrimaryKeyConstraint('old_id'),
    sa.UniqueConstraint('new_id')
