"""baseline_migration_corrected_foreign_keys

Revision ID: 9164fb08a156
Revises: 
Create Date: 2025-07-30 17:44:37.220988+00:00
...
def upgrade() -> None:
    ...
    op.add_column('inventory', sa.Column('storage_id', sa.UUID(), nullable=True, comment='FK hacia Storage - ubicación física del inventario'))
    ...
    op.alter_column('inventory', 'deleted_at',
               ...
               existing_nullable=True)
    op.create_foreign_key(None, 'inventory', 'storages', ['storage_id'], ['id'])
    ...
    op.alter_column('products', 'deleted_at',
               ...
               existing_comment='Fecha de eliminación lógica (soft delete)',
               existing_nullable=True)
    op.add_column('storages', sa.Column('vendedor_id', sa.UUID(), nullable=True, comment='ID del usuario vendedor propietario del espacio'))
    op.add_column('storages', sa.Column('productos_actuales', sa.Integer(), nullable=False, comment='Número actual de productos almacenados'))
    op.add_column('storages', sa.Column('ocupacion_actual', sa.DECIMAL(precision=5, scale=2), nullable=False, comment='Porcentaje actual de ocupación (0.00 a 100.00)'))
    op.add_column('storages', sa.Column('ultima_actualizacion', sa.DateTime(), nullable=True, comment='Fecha de última actualización del tracking'))
    op.add_column('storages', sa.Column('fecha_inicio', sa.DateTime(), nullable=True, comment='Fecha de inicio del contrato de almacenamiento'))
    op.add_column('storages', sa.Column('fecha_fin', sa.DateTime(), nullable=True, comment='Fecha de finalización del contrato de almacenamiento'))
    op.add_column('storages', sa.Column('renovacion_automatica', sa.Boolean(), nullable=False, comment='Indica si el contrato se renueva automáticamente'))
    ...
    op.create_foreign_key(None, 'storages', 'users', ['vendedor_id'], ['id'])
    op.add_column('transactions', sa.Column('inventory_id', sa.UUID(), nullable=True, comment='ID del inventario específico involucrado en la transacción'))
    ...
    op.create_foreign_key(None, 'transactions', 'inventory', ['inventory_id'], ['id'])
...
def downgrade() -> None:
    ...
    op.alter_column('transactions', 'monto',
               ...
               existing_comment='Monto de la transacción en pesos colombianos (COP)',
               existing_nullable=False)
    op.drop_column('transactions', 'inventory_id')
    op.drop_constraint(None, 'storages', type_='foreignkey')
    ...
    op.drop_column('storages', 'ultima_actualizacion')
    op.drop_column('storages', 'ocupacion_actual')
    op.drop_column('storages', 'productos_actuales')
    op.drop_column('storages', 'vendedor_id')
    op.alter_column('products', 'deleted_at',
               existing_type=sa.DateTime(),
               type_=postgresql.TIMESTAMP(),
               existing_comment='Fecha de eliminación lógica (soft delete)',
